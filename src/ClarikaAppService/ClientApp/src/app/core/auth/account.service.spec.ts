  ...
  describe("identity", () => {
    ...
    describe("should change the language on authentication if necessary", () => {
      it("should change language if user has not changed language manually", () => {
        // GIVEN
        sessionStorageService.retrieve = jest.fn((key) =>
          key === "locale" ? undefined : "otherSessionStorageValue"
        );
        ...
        httpMock
          ...
          .flush({ ...accountWithAuthorities([]), langKey: "accountLang" });
      ...
      it("should not change language if user has changed language manually", () => {
        // GIVEN
        sessionStorageService.retrieve = jest.fn((key) =>
          key === "locale" ? "sessionLang" : undefined
        );
