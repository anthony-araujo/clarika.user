...
export class StateService {
  protected resourceUrl =
    this.applicationConfigService.getEndpointFor("api/states");
  ...
  update(state: IState): Observable<EntityResponseType> {
    return this.http.put<IState>(
      `${this.resourceUrl}/${getStateIdentifier(state) as number}`,
      state,
      { observe: "response" }
    );
  }
  ...
  find(id: number): Observable<EntityResponseType> {
    return this.http.get<IState>(`${this.resourceUrl}/${id}`, {
      observe: "response",
    });
  }
  ...
  query(req?: any): Observable<EntityArrayResponseType> {
    ...
    return this.http.get<IState[]>(this.resourceUrl, {
      params: options,
      observe: "response",
    });
  }
  ...
  addStateToCollectionIfMissing(
    stateCollection: IState[],
    ...statesToCheck: (IState | null | undefined)[]
  ): IState[] {
    const states: IState[] = statesToCheck.filter(isPresent);
    if (states.length > 0) {
      const stateCollectionIdentifiers = stateCollection.map(
        (stateItem) => getStateIdentifier(stateItem)!
      );
      const statesToAdd = states.filter((stateItem) => {
        const stateIdentifier = getStateIdentifier(stateItem);
        if (
          stateIdentifier == null ||
          stateCollectionIdentifiers.includes(stateIdentifier)
        ) {
          return false;
        }
