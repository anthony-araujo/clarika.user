...
/**
 * Performance test for the UserApp entity.
 */
class UserAppGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the UserApp entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all userApps")
            .get("/api/user-apps")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new userApp")
            .post("/api/user-apps")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                ...
                , "email":"SAMPLE_TEXT"
                , "dateBirth":"2020-01-01T00:00:00.000Z"
                , "age":"0"
                , "passwordHash":"SAMPLE_TEXT"
                , "securityStamp":"SAMPLE_TEXT"
                , "concurrencyStamp":"SAMPLE_TEXT"
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_userApp_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created userApp")
                .get("${new_userApp_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created userApp")
            .delete("${new_userApp_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
