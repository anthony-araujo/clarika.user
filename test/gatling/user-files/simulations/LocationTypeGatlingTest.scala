...
/**
 * Performance test for the LocationType entity.
 */
class LocationTypeGatlingTest extends Simulation {
    ...
    val baseURL = Option(System.getProperty("baseURL")) getOrElse """http://localhost:5000"""
    ...
    val scn = scenario("Test the LocationType entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all locationTypes")
            .get("/api/location-types")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new locationType")
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_locationType_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created locationType")
                .get("${new_locationType_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created locationType")
            .delete("${new_locationType_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
