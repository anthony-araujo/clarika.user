...
/**
 * Performance test for the UserLocation entity.
 */
class UserLocationGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the UserLocation entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all userLocations")
            .get("/api/user-locations")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new userLocation")
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_userLocation_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created userLocation")
                .get("${new_userLocation_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created userLocation")
            .delete("${new_userLocation_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
